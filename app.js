...
// Keep this option visible in the Category dropdown even before the sheet
// contains an approved non-serviced villa row.
const CATEGORY_ALWAYS_AVAILABLE = ["Non-Serviced Villa"];
...
const CATEGORY_ALIASES = {
  ...
  "villa": "Villa",
  "villa non serviced": "Non-Serviced Villa",
  "villa non-serviced": "Non-Serviced Villa",
  "non serviced villa": "Non-Serviced Villa",
  "non-serviced villa": "Non-Serviced Villa"
};
...
async function loadData() {
  ...
  // Sample data is reserved for an explicitly marked visual preview only.
  if (CONFIG.USE_SAMPLE_DATA === true) {
    return { ...SAMPLE, _sampleData: true };
  }

  if (!url || url.includes("PASTE_YOUR")) {
    throw new Error("Live data URL is missing. Add the Google Apps Script /exec URL in config.js.");
  }

  // Apps Script uses JSONP first because it avoids cross-origin fetch restrictions on GitHub Pages.
  if (isAppsScriptUrl(url)) {
    try {
      ...
      try {
        ...
        console.error("Live catalogue data could not be loaded.", fetchError);
        throw new Error("The live catalogue could not be loaded. Check that the Apps Script web app is deployed and its access is set to allow the catalogue website.");
      }
    }
  }
  ...
  try {
    ...
    try {
      ...
      console.error("Live catalogue data could not be loaded.", jsonpError);
      throw new Error("The live catalogue could not be loaded. Check the data URL and its access settings.");
    }
  }
}
...
function orderedCategoryOptions(rows) {
  // Keep selected core categories visible even when no current sheet row uses them.
  const categories = [...new Set([
    ...uniqueOptions(rows, "Category"),
    ...CATEGORY_ALWAYS_AVAILABLE
  ])];
...
async function init() {
  ...
  try {
    ...
    if (!payload || payload.ok === false) {
      throw new Error((payload && payload.error) || "The catalogue API returned no usable data.");
    }

    catalogueRows = normalizeRows(payload.rows || []);

    fillSelect(els.area, uniqueOptions(catalogueRows, "Area"), "All areas");
    fillSelect(els.category, orderedCategoryOptions(catalogueRows), "All categories");
    ...
    els.updatedAt.textContent = payload._sampleData
      ? "Preview data only"
      : updated && !Number.isNaN(updated.getTime())
        ? `Live data refreshed: ${updated.toLocaleString()}`
        : "Live data loaded";
    ...
    fillSelect(els.area, [], "All areas");
    fillSelect(els.category, orderedCategoryOptions([]), "All categories");
    fillSelect(els.bedrooms, [], "Any bedroom");
    renderStats([], []);
    renderCompare();

    els.updatedAt.textContent = "Live data unavailable";
    els.grid.innerHTML = `
      <div class="empty">
        <strong>Live catalogue data could not be loaded.</strong><br>
        ${escapeHtml(error.message || "Check the Apps Script configuration and try again.")}<br>
        Confirm the <code>DATA_URL</code> in <code>config.js</code> and the Apps Script web-app deployment, then choose <strong>Refresh data</strong>.
      </div>
    `;
  }
}
